...
function prepare_dataset(n_samples=50_000, split_ratio=0.8, max_len=130)
    ...
    path = joinpath(@__DIR__, "../../data/imdb_dataset.jld2")
    reviews = load(path, "reviews")[1:n_samples]
    labels = load(path, "labels")[1:n_samples]
    ...
    embeddings_path = joinpath(@__DIR__, "../../data/glove_6B_50d.jld2")
    embeddings = load(embeddings_path, "embeddings")
    ...
    output_path = joinpath(@__DIR__, "../../data/embeddings/imdb_dataset_prepared.jld2")
    @info "Saving prepared data to $output_path ..."
    jldsave(output_path;
        X_train=X_train,
        y_train=y_train,
        X_test=X_test,
